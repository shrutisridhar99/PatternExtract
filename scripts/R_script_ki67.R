...
## ----------------------------- USER PATHS ----------------------------------#
# Accept data_dir from command line argument or environment variable
args_cli <- commandArgs(trailingOnly = TRUE)
data_dir <- if (length(args_cli) > 0) {
  args_cli[1]
} else {
  Sys.getenv("PATTERNEXTRACT_DATA",
             unset = file.path(dirname(getwd()), "data"))
}
cat(sprintf("Using data directory: %s\n", data_dir))
